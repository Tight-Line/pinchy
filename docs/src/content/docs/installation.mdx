...
# Better Auth session secret (default: dev-secret-change-in-production)
...
**`BETTER_AUTH_SECRET`** — Used by Better Auth for session security. Generate one with `openssl rand -hex 32`.
